...
function ChatWindow() {
  ...
  const chat = getMessages(phone);
  const messages = getMessages(phone) || [];
  const isTyping = typingStatus[phone];
  const onlineState = onlineStatus[phone] || 'offline';

    // Handle menu button clicks from ChatBubble
  const handleMenuButtonClick = async (clickData) => {
    const { phone: btnPhone, buttonId, buttonTitle } = clickData;
    console.log(`🔘 Menu button clicked: ${buttonId} for ${btnPhone}`);
    
    try {
      const response = await messageAPI.triggerAction(btnPhone, buttonId);
      if (response?.success) {
        showToast(`Triggered: ${buttonTitle}`, 'success');
        fetchData();
      } else {
  ...
  const generateAiSuggestions = useCallback((messagesList) => {
  // Safety check - ensure messagesList is an array
  ...
  if (!dateMessages || !Array.isArray(dateMessages)) return null;
